...
public class DiasSpinnerAdapter extends ArrayAdapter<Dias> {

    private Context context;
    List<Dias> datos = null;

    public DiasSpinnerAdapter(Context context, List<Dias> datos)
    {
    ...
    public View getView(int position, View convertView, ViewGroup parent)
    {
        ...
        ((TextView) convertView.findViewById(R.id.texto)).setText(datos.get(position).getDesc());
    ...
    public View getDropDownView(int position, View convertView, ViewGroup parent) {
        ...
        if (row.getTag() == null)
        {
            DiasHolder tipoRecHolder= new DiasHolder();
            tipoRecHolder.setTextView((TextView) row.findViewById(R.id.texto));
            row.setTag(tipoRecHolder);
        }

        //rellenamos el layout con los datos de la fila que se está procesando
        Dias dia = datos.get(position);
        ((DiasHolder) row.getTag()).getTextView().setText(dia.getDesc());

        return row;
    }

    private static class DiasHolder {
